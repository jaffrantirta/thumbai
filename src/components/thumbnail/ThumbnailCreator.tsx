...
import {
  Sparkles, Download, Loader2, ImageIcon, LayoutTemplate,
  User, ImageOff, Upload, X,
  // vibe icons
  Zap, Smile, Laugh, Frown, Flame, AlertTriangle, Star,
  Target, Crown, HelpCircle, Coffee, AlertCircle,
  // template icons
  Layers, AlignLeft, Bold, Briefcase,
  type LucideIcon,
} from "lucide-react";
...
const TEMPLATES: { id: TemplateId; icon: LucideIcon; label: string }[] = [
  { id: "modern",    icon: Layers,    label: "modern"    },
  { id: "minimal",   icon: AlignLeft, label: "minimal"   },
  { id: "bold",      icon: Bold,      label: "bold"      },
  { id: "neon",      icon: Zap,       label: "neon"      },
  { id: "corporate", icon: Briefcase, label: "corporate" },
];

const VIBES: { value: string; icon: LucideIcon; label: string }[] = [
  { value: "excited",    icon: Zap,           label: "excited"    },
  { value: "happy",      icon: Smile,         label: "happy"      },
  { value: "shocked",    icon: AlertCircle,   label: "shocked"    },
  { value: "funny",      icon: Laugh,         label: "funny"      },
  { value: "sad",        icon: Frown,         label: "sad"        },
  { value: "angry",      icon: Flame,         label: "angry"      },
  { value: "scared",     icon: AlertTriangle, label: "scared"     },
  { value: "inspiring",  icon: Star,          label: "inspiring"  },
  { value: "determined", icon: Target,        label: "determined" },
  { value: "confident",  icon: Crown,         label: "confident"  },
  { value: "curious",    icon: HelpCircle,    label: "curious"    },
  { value: "chill",      icon: Coffee,        label: "chill"      },
];
...
function GeneratingAnimation({ ratio }: { ratio: Ratio }) {
  return (
    <div className={cn("relative overflow-hidden rounded-xl bg-zinc-900", RATIO_CLASSES[ratio])}>
      {/* shimmer sweep */}
      ...
      {/* ambient glow orbs */}
      <div className="absolute top-1/4 left-1/4 w-40 h-40 rounded-full bg-violet-600/20 blur-3xl animate-pulse" />
      <div className="absolute bottom-1/4 right-1/4 w-28 h-28 rounded-full bg-violet-400/10 blur-2xl animate-pulse [animation-delay:700ms]" />
      {/* bouncing dots */}
        ...
        <div className="flex items-end gap-1.5 h-5">
          {[0, 1, 2, 3].map((i) => (
            <div
              ...
              style={{
                height: "6px",
                animation: `bounce-dot 1.1s ease-in-out ${i * 0.14}s infinite`,
              }}
            />
          ))}
        </div>
        <p className="text-zinc-500 text-xs tracking-widest">generating</p>
      </div>
    </div>
  );
}
...
export function ThumbnailCreator() {
  ...
  const [mode,        setMode]        = useState<Mode>("image-gen"); // default AI

  // image-gen only
  const [includeSubject, setIncludeSubject] = useState(true);
  const [faceBase64,     setFaceBase64]     = useState("");
  const [facePreview,    setFacePreview]    = useState("");
  const [selectedVibe,   setSelectedVibe]   = useState("excited");
  ...
  async function handleFaceFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setFaceBase64(b64);
      setFacePreview(b64);
    } catch { setGenError("failed to process image"); }
  }

  function clearFace() {
    setFaceBase64(""); setFacePreview("");
  ...
  async function handleAiGenerate() {
    ...
    setGenError(""); setGenerating(true); setImageUrl("");
    try {
      ...
      const genResp = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          ...
          vibe: selectedVibe, thumbnailId: created.id,
        }),
      });
      const genData = await genResp.json();
      if (!genResp.ok) throw new Error(genData.error || "generation failed");
      if (!genData.imageUrl) throw new Error("no image returned — check your image model in settings");
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {(["image-gen", "template"] as Mode[]).map((m) => (
              <button key={m} onClick={() => { setMode(m); setImageUrl(""); setGenError(""); }}
        ...
        {/* ── design mode only options ── */}
        {mode === "template" && (
          <>
            {/* color — only for design */}
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-5 gap-1.5">
                {TEMPLATES.map(({ id, icon: Icon, label }) => (
                  <button key={id} onClick={() => setTemplate(id)}
                    className={cn("flex flex-col items-center py-3 px-1 rounded-lg border text-xs font-medium transition-all gap-1.5",
                      template === id ? "border-violet-500/60 bg-violet-500/10 text-white" : "border-zinc-800 bg-zinc-900 text-zinc-500 hover:border-zinc-700"
                    )}
                  >
              ...
              <div className="space-y-1.5">
                ...
                {facePreview ? (
                  ...
                  >
                    ...
                    <span className="text-xs opacity-50">jpg / png · max 5 mb</span>
                  </button>
                )}
                <input ref={faceInputRef} type="file" accept="image/*" className="hidden" onChange={handleFaceFile} />
              </div>
            )}
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {VIBES.map(({ value, icon: Icon, label }) => (
                  <button key={value} onClick={() => setSelectedVibe(value)}
                    className={cn(
                      "flex flex-col items-center py-3 rounded-lg border text-xs font-medium transition-all gap-1.5",
                      selectedVibe === value
                        ? "border-violet-500/60 bg-violet-500/10 text-white"
                        : "border-zinc-800 bg-zinc-900 text-zinc-500 hover:border-zinc-700 hover:text-zinc-300"
                    )}
      ...
      <div className="space-y-3">
        ...
        <div className="bg-zinc-950 border border-zinc-900 rounded-xl p-3">
          {mode === "image-gen" && generating ? (
            <GeneratingAnimation ratio={ratio} />
          ) : (
            <ThumbnailPreview
              title={title}
        ...
        {mode === "image-gen" && generating && (
          <p className="text-zinc-600 text-xs flex items-center gap-2">
            {faceBase64 ? "describing face then generating..." : "creating your thumbnail with ai..."}
          </p>
        )}
      </div>
    </div>
